...
from iirc.iirc_definitions import IIRC_SETUP, NO_LABEL_PLACEHOLDER, CIL_SETUP
...
def test_LLLDataset_divide_data_across_tasks_IIRC_Train(expected_task_data_size):
    lll_dataset = TorchLLLDataset(dataset, tasks, essential_transforms_fn=lambda x: x, setup=IIRC_SETUP, test_mode=False,
                                  superclass_data_pct=superclass_data_pct, subclass_data_pct=subclass_data_pct)
    for task_id in range(len(tasks)):
        assert len(lll_dataset.task_id_to_data_idx[task_id]) == expected_task_data_size[task_id]


@pytest.mark.parametrize("expected_task_data_size", [[500, 400, 300]])
def test_LLLDataset_divide_data_across_tasks_IIRC_Test(expected_task_data_size):
    lll_dataset = TorchLLLDataset(dataset, tasks, test_mode=True, setup=IIRC_SETUP, essential_transforms_fn=lambda x: x)
    for task_id in range(len(tasks)):
        assert len(lll_dataset.task_id_to_data_idx[task_id]) == expected_task_data_size[task_id]


@pytest.mark.parametrize("expected_task_data_size", [[500, 400, 300]])
def test_LLLDataset_divide_data_across_tasks_CIL(expected_task_data_size):
    lll_dataset = TorchLLLDataset(dataset, tasks, setup=CIL_SETUP, essential_transforms_fn=lambda x: x)
    for task_id in range(len(tasks)):
        assert len(lll_dataset.task_id_to_data_idx[task_id]) == expected_task_data_size[task_id]
...
def test_choose_task(expected_task_data_size):
    lll_dataset = TorchLLLDataset(dataset, tasks, essential_transforms_fn=lambda x: x, setup=IIRC_SETUP,
                                  superclass_data_pct=superclass_data_pct, subclass_data_pct=subclass_data_pct)

    for task_id in range(len(tasks)):
        lll_dataset.choose_task(task_id)
        assert len(lll_dataset) == expected_task_data_size[task_id]
        assert set(lll_dataset.seen_classes) == set([cla for task in tasks[:task_id + 1] for cla in task])

        # test that the no labels from outside the current task are given (when not using complete information mode in
        # the case of IIRC)
        assert lll_dataset.cur_task == tasks[task_id]
        for i in range(len(lll_dataset)):
            image, label_1, label_2 = lll_dataset[i]
...
def test_complete_information_mode(expected_task_data_size, expected_data_up_to_size):
    ...
    for task_id in range(len(tasks)):
        ...
        for i in range(len(lll_dataset)):
            image, label_1, label_2 = lll_dataset[i]
            assert label_1 == NO_LABEL_PLACEHOLDER or label_1 in lll_dataset.seen_classes
            assert label_2 == NO_LABEL_PLACEHOLDER or label_2 in lll_dataset.seen_classes
        ...
        lll_dataset.load_tasks_up_to(task_id)
        assert len(lll_dataset) == expected_data_up_to_size[task_id]
        assert set(lll_dataset.cur_task) == set([cla for task in tasks[:task_id + 1] for cla in task])
        for i in range(len(lll_dataset)):
            image, label_1, label_2 = lll_dataset[i]
            assert label_1 == NO_LABEL_PLACEHOLDER or label_1 in lll_dataset.seen_classes
            assert label_2 == NO_LABEL_PLACEHOLDER or label_2 in lll_dataset.seen_classes
...
def test_get_shuffled_image_indices(class1, class2, class1_size0, class1_size01, class2_size1, class2_size01):
    ...
    lll_dataset.choose_task(0)
    class1_indices = lll_dataset.get_image_indices_by_cla(class1)
    assert len(class1_indices) == class1_size0
    for idx in class1_indices:
        image, label_1, label_2 = lll_dataset[idx]
        assert label_1 == class1
        assert label_2 == NO_LABEL_PLACEHOLDER

    lll_dataset.choose_task(1)
    class2_indices = lll_dataset.get_image_indices_by_cla(class2)
    ...
    lll_dataset.enable_complete_information_mode()
    lll_dataset.load_tasks_up_to(1)
    class1_indices = lll_dataset.get_image_indices_by_cla(class1)
    class2_indices = lll_dataset.get_image_indices_by_cla(class2)
