...
from iirc.iirc_definitions import PYTORCH, TENSORFLOW, CIL_SETUP, IIRC_SETUP, DatasetStructType
from iirc.utils.utils import unpickle, print_msg
from iirc.lll_dataset.lll_dataset import LLLDatasetBase

metadata_root = os.path.join(os.path.abspath("."), "./iirc/metadata")
...
def get_lll_datasets(dataset_name: str,
    ...
    Dict[str, LLLDatasetBase], List[List[str]], Dict[str, int]]:
    """
    ...
    Returns:
        Tuple[Dict[str, LLLDatasetBase], List[List[str]], Dict[str, int]]:

        lll_datasets (Dict[str, LLLDatasetBase]): a dictionary with the keys corresponding to the four splits (train,
        intask_validation, posttask_validation, test), and the values containing the dataset object inheriting from
        LLLDatasetBase for that split.
    ...
    if framework == PYTORCH:
        ...
        LLLDataset = TorchLLLDataset
    elif framework == TENSORFLOW:
        from iirc.lll_dataset.tensorflow_lll_dataset import TensorflowLLLDataset
        LLLDataset = TensorflowLLLDataset
    else:
    ...
    lll_datasets["train"] = LLLDataset(dataset=datasets["train"], test_mode=False,
                                       augmentation_transforms_fn=augmentation_transforms_fn, **shared_arguments)
    lll_datasets["intask_valid"] = LLLDataset(dataset=datasets["intask_valid"], test_mode=False, **shared_arguments)
    lll_datasets["posttask_valid"] = LLLDataset(dataset=datasets["posttask_valid"], test_mode=True, **shared_arguments)
    lll_datasets["test"] = LLLDataset(dataset=datasets["test"], test_mode=True, **shared_arguments)

    print_msg("Dataset created")

    return lll_datasets, tasks, class_names_to_idx
