...
class BufferBase(ABC, data.Dataset):
    ...
    @abstractmethod
    def _construct_exemplar_set(self, task_data: TorchLLLDataset, **kwargs) -> None:
        """update the buffer with the new task exemplars (implement in the Buffer class in the method file)"""
        pass

    def update_buffer_new_task(self, new_task_data: TorchLLLDataset, dist_args: Optional[Dict] = None,
                               **kwargs) -> None:
        """
...
class TaskDataMergedWithBuffer(data.Dataset):
    ...
    def __init__(self, buffer: BufferBase, task_data: TorchLLLDataset, buffer_sampling_multiplier: float = 1.0):
        self.buffer = buffer
