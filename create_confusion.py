...
from lll_methods.utils import SubsetSampler
...
def get_model(args, device, task_id=0):
    if args.models_dir == "None":
        ...
        model = method.Model(metadata["n_cla_per_tsk"], metadata["class_names_to_idx"], config) # type: Optional[LLLMethodBase]
        model.to(device)
...
if __name__ == '__main__':
    ...
    lll_datasets, tasks, class_names_to_idx = \
        get_lll_datasets(args.dataset, dataset_root=args.dataset_path,
    ...
    dataset = lll_datasets["posttask_valid"]
    model = None
    
    tasks_id_to_show = [0, 1, 5, 10]
    num_classes_per_task = [10, 5, 5, 5]
    confusions = [np.zeros((sum(num_classes_per_task), sum(num_classes_per_task))) for _ in range(len(tasks_id_to_show))]
    ...
    for task_id, confusion in zip(tasks_id_to_show, confusions):
        confusion_df = pd.DataFrame(confusion, total_classes_to_show,
                                    total_classes_to_show)
        matplotlib.use('Agg')
        sn.heatmap(confusion_df*100.0, vmin=0, vmax=100, annot=False, yticklabels=False, xticklabels=False, fmt='.1f')  # , annot_kws={"size": 8}
