...
from lll_methods.buffer.buffer import TaskDataMergedWithBuffer
from lll_methods.utils import transform_labels_names_to_vector
from utility import utils, metrics
...
def task_train(model, buffer, lll_datasets, config, metadata, logbook, dist_args=None):
    ...
    best_checkpoint = {
        "model_state_dict": utils.deepcopy(model.method_state_dict()),
    ...
    task_train_data = lll_datasets['train']
    if config["method"] == "agem":
        ...
        task_train_data_with_buffer = TaskDataMergedWithBuffer(buffer, task_train_data, buffer_sampling_multiplier=bsm)
    task_valid_data = lll_datasets['intask_valid']
    ...
    for epoch in range(config['task_epoch'], num_epochs):
        ...
        if config['use_best_model']:
            if log_dict[f"valid_modified_jaccard_{cur_task_id}"] >= best_checkpoint["best_modified_jaccard"]:
                ...
                best_checkpoint["model_state_dict"] = utils.deepcopy(model.method_state_dict())
...
def tasks_eval(model, dataset, cur_task_id, config, metadata, logbook, dataset_type="valid", dist_args=None):
    ...
    if rank == 0:
        ...
        metrics_dict["rank"] = rank
        print_msg(metrics_dict)
    # if cur_task_id <= 1:  # For analysis
        # evaluate_confusion(model, dataset, cur_task_id, config, metadata)


def evaluate_confusion(model, dataset, cur_task_id, config, metadata):
    class_names_to_idx = metadata["class_names_to_idx"]
    idx_to_class_names = ["" for _ in range(len(class_names_to_idx))]
    for class_name, idx in class_names_to_idx.items():
        idx_to_class_names[idx] = class_name
    model.net.eval()
    num_seen_classes = len(model.seen_classes)
    confusion = np.zeros((num_seen_classes, num_seen_classes))
    normalization_const = np.zeros((num_seen_classes, num_seen_classes))
    dataset.load_tasks_up_to(cur_task_id)
    data_loader = data.DataLoader(dataset, batch_size=config["batch_size"], shuffle=False,
                                  num_workers=config["num_workers"])
    with torch.no_grad():
        for minibatch in data_loader:
            images = minibatch[0].to(config["device"])
            labels_names = list(zip(minibatch[1], minibatch[2]))
            labels = transform_labels_names_to_vector(labels_names, len(dataset.seen_classes),
                                                      class_names_to_idx).to(config["device"])

            predictions = model(images)
            target = labels.bool()

            for i in range(num_seen_classes):  # True class
                class_idx = target[:, i] == True
                target_ = target[class_idx, :]
                predictions_ = predictions[class_idx, :]
                for j in range(num_seen_classes):
                    if j > i:
                        # The true label is a superclass, while the comparison label is one of its subclasses
                        confusion[i, j] += torch.sum((predictions_[:, j] == True) & (target_[:, j] == False)).item()
                        normalization_const[i, j] += target_.shape[0] - torch.sum(target_[:, j] == True).item()
                    elif i == j or j < i:  # assumes that no subclass is introduced before its own superclass
                        confusion[i, j] += torch.sum(predictions_[:, j] == True)
                        normalization_const[i, j] += target_.shape[0]

    confusion /= normalization_const

    confusion_df = pd.DataFrame(confusion * 100., idx_to_class_names[:num_seen_classes],
                                idx_to_class_names[:num_seen_classes])
    matplotlib.use('Agg')
    sn.heatmap(confusion_df, vmin=0, vmax=100, annot=True, fmt='.1f')  # , annot_kws={"size": 8}
    fig = matplotlib.pyplot.gcf()
    fig.set_size_inches(15, 12)
    fig.tight_layout()
    plt.savefig(os.path.join(config["logging_path"], f"task_{cur_task_id}_confusion.png"))
    plt.close()
